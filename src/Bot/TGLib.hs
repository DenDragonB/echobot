...
getResponseFromAPI token settings = do
    --request <- parseRequest "https://api.telegram.org/bot1601854063:AAHSq7CxULiYMUyHiSiwS1ByOVA-kUg_ejU/getUpdates?timeout=60&offset=988265815"
    let request
            = setRequestMethod (BS.fromString "GET")
            $ setRequestHost   (BS.fromString "api.telegram.org")
            $ setRequestPort   (443)
            $ setRequestSecure (True)
            $ setRequestPath   (BS.fromString $ "/bot" ++ token ++ "/" ++ tgMethod settings)
            $ setRequestQueryString (tgReqParams settings)
            $ defaultRequest
    res <- httpBS request
    return (getResponseBody res)

-- Methods of Telegram API
tgGetUpdates :: Integer -> Integer -> ReqSet
tgGetUpdates timeout offset = ReqSet {tgMethod = "getUpdates",
    tgReqParams = [(BS.fromString "timeout", Just $ BS.fromString $ show timeout),
                   (BS.fromString "offset",  Just $ BS.fromString $ show offset)]}

tgSendMessage :: Integer -> String -> ReqSet
tgSendMessage chatID
              ...
              --replyID
              --replyMarkUp 
              =  ReqSet {tgMethod = "sendMessage",
                  tgReqParams = [(BS.fromString "chat_id", Just $ BS.fromString $ show chatID)
                                ,(BS.fromString "text", Just $ BS.fromString $ text)
                                 --,(BS.pack "parse_mode", Just $ BS.pack $ show timeout)
                                 --,(BS.pack "disable_web_page_preview", Just $ BS.pack $ show timeout)
                                 --,(BS.pack "disable_notification", Just $ BS.pack $ show timeout)
                                 --,(BS.pack "reply_to_message_id", Just $ BS.pack $ show timeout)
                                 --,(BS.pack "offset",  Just $ BS.pack $ show offset)
                                 ]}

tgSendButtons :: Integer -> String -> TGReplyKeyboardMarkup -> ReqSet
tgSendButtons chatID
              text
              --parse
              --preview
              --notification
              ...
              =  ReqSet {tgMethod = "sendMessage",
                  tgReqParams = [(BS.fromString "chat_id", Just $ BS.fromString $ show chatID)
                                ,(BS.fromString "text", Just $ BS.fromString $ text)
                                ,(BS.fromString "reply_markup", Just $ BSLazy.toStrict $ encode replyMarkUp)
                                ]}

tgSendPhoto :: Integer -> String -> ReqSet
tgSendPhoto chatID
            photo
            -- caption
            -- notification
            -- replyID
            -- replyMarkUp
            = ReqSet {tgMethod = "sendPhoto",
                      tgReqParams = [(BS.fromString "chat_id", Just $ BS.fromString $ show chatID)
                                    ,(BS.fromString "photo", Just $ BS.fromString $ photo)
                                    --,(BS.pack "caption", Just $ BS.pack $ show caption)
                                    --,(BS.pack "disable_notification", Just $ BS.pack $ show notification)
                                    --,(BS.pack "reply_to_message_id", Just $ BS.pack $ show replyID)
                                    --,(BS.pack "reply_markup", Just $ BS.pack $ show replyMarkUp)
                                    ]}

tgCopyMessage :: Integer -> Integer -> Integer -> ReqSet
tgCopyMessage chatID
              fromChatID
              messageID
              = ReqSet {tgMethod = "copyMessage",
                        tgReqParams = [(BS.fromString "chat_id", Just $ BS.fromString $ show chatID)
